        ...
        <>
            ...
            <div className="max-w-[420px] mx-auto p-3">
                <div className="bg-panel border border-border rounded-xl shadow-lg flex flex-col max-h-[600px] overflow-hidden">
                    {showTopbar && (
                        <div
                            className="sticky top-0 z-10 flex justify-center gap-3 px-2 pt-2"
                            style={{
                                background:
                                    'linear-gradient(180deg,rgba(64,24,92,.8),rgba(64,24,92,0))',
                            }}
                        >
                            <div className="bg-purple-dark text-white border border-white/20 rounded-full px-3.5 py-1.5 font-extrabold inline-flex items-center gap-2 shadow-[inset_0_0_0_1px_rgba(255,255,255,.08)]">
                                <span>💸</span>
                                <span>Derash {Math.round(derash)} ETB</span>
                            </div>
                  ...
                  ${
                      taken.has(String(n))
                          ...
                          : 'bg-accent text-stone-400 border border-accent-hover cursor-pointer hover:brightness-110 active:scale-95'
                                ...
                                onClick={() =>
                                    !taken.has(String(n)) && setModalIndex(n)
                                }
                            >
                                {n}
                            </div>
                    ...
                    <div className="p-3.5 border-t border-border flex justify-center">
                        <button
                            className="bg-blue-bingo border border-[#1d6fe6] text-white px-4 py-2.5 rounded-full font-semibold cursor-pointer hover:brightness-110 active:scale-95 transition"
                ...
                <div className="text-muted text-xs text-center mt-3">
            ...
            {modalIndex && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999] backdrop-blur-lg">
                    <div
                        className="w-[360px] max-w-[95vw] rounded-2xl shadow-2xl p-3.5"
                        style={{
                            background: 'linear-gradient(180deg,#f6a623,#d8890b)',
                        }}
                    >
                        <div className="w-9 h-9 rounded-full bg-[#ffcc33] flex items-center justify-center mx-auto mb-2 font-extrabold text-[#7a4b00] border-2 border-[#f0b000]">
                            {modalIndex}
                        </div>
                    ...
                    ${
                        ...
                        <div className="flex gap-2.5 justify-center mt-2.5">
                            <button
                                className="bg-green-bingo border-none text-[#04120a] px-4 py-2.5 rounded-full font-bold cursor-pointer hover:brightness-110 active:scale-95 transition"
                                onClick={handleAccept}
                            >
                                Accept
                            </button>
                            <button
                                className="bg-gray-400 border-none text-gray-900 px-4 py-2.5 rounded-full font-bold cursor-pointer hover:brightness-110 active:scale-95 transition"
            ...
            {showInsufficient && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999] backdrop-blur-lg">
                    <div className="w-[360px] max-w-[90vw] bg-white text-gray-900 rounded-xl shadow-2xl p-4">
                        ...
                        <button
                            className="bg-gold-gradient-to text-white border-none rounded-lg px-3.5 py-2 font-bold cursor-pointer hover:brightness-110 transition"
