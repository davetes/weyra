...
export default function PlayPage() {
    ...
    async function handleAccept() {
        ...
        form.set('index', String(modalIndex));
    ...
    return ( <
            ...
            <
            div className = "bg-panel border border-border rounded-xl shadow-lg flex flex-col max-h-[600px] overflow-hidden" > { /* Topbar */ } {
                showTopbar && ( <
                    div className = "sticky top-0 z-10 flex justify-center gap-3 px-2 pt-2"
                    style = {
            ...
            div className = "flex justify-between items-center font-bold mx-auto gap-2 flex-wrap px-3 w-full my-2" > {
                [
                    ...
                    div key = { p.label }
                    className = "flex flex-col items-center border border-border text-white px-2.5 py-2 rounded-[10px] text-xs flex-1 text-center bg-purple-bright whitespace-nowrap" >
                    <
            ...
            div className = "grid grid-cols-10 gap-1 p-3 overflow-y-auto flex-grow" > {
                numbers.map(n => ( <
                  ...
                  ${taken.has(String(n))
                    ...
                    onClick = {
                        () => !taken.has(String(n)) && setModalIndex(n) } > { n } <
                    /div>
                ))
            } <
            /div>
            ...
            <
            button className = "bg-blue-bingo border border-[#1d6fe6] text-white px-4 py-2.5 rounded-full font-semibold cursor-pointer hover:brightness-110 active:scale-95 transition"
            onClick = {
                () => location.reload() } >
            Refresh <
            /button> <
            ...
            { /* Card Preview Modal */ } {
                modalIndex && ( <
                    ...
                    <
                    div className = "w-[360px] max-w-[95vw] rounded-2xl shadow-2xl p-3.5"
                    style = {
                    ...
                    div className = "grid grid-cols-5 gap-1.5 mb-1.5" > {
                        LETTERS.map(l => < div key = { l }
                            className = { `${LETTER_BG[l]} text-white font-extrabold text-center py-1.5 rounded-md` } > { l } < /div>)} <
                            /div> <
                    ...
                    ${val === 'FREE' ? 'bg-card-free border-card-free-border text-white' : 'bg-white border-gray-300 text-black'}` } > { val === 'FREE' ? '⭐' : val } <
                                    /div>
                                ))
                            } <
                            /div> <
                            ...
                            <
                            button className = "bg-green-bingo border-none text-[#04120a] px-4 py-2.5 rounded-full font-bold cursor-pointer hover:brightness-110 active:scale-95 transition"
                            onClick = { handleAccept } > Accept < /button> <
                            button className = "bg-gray-400 border-none text-gray-900 px-4 py-2.5 rounded-full font-bold cursor-pointer hover:brightness-110 active:scale-95 transition"
                            onClick = {
                                () => setModalIndex(null) } > Cancel < /button> <
                            /div> <
                            /div> <
                            /div>
                        )
                    ...
                    { /* Insufficient balance dialog */ } {
                        showInsufficient && ( <
                            ...
                            button className = "bg-gold-gradient-to text-white border-none rounded-lg px-3.5 py-2 font-bold cursor-pointer hover:brightness-110 transition"
                            onClick = {
                                () => setShowInsufficient(false) } > Close < /button> <
                            /div> <
                            /div>
                        )
                    } <
