...
const NAV = [
    ...
    { href: '/deposit', label: 'Deposits', perm: 'deposit.read', icon: IconDeposit, badge: 'pendingDeposits' },
    { href: '/withdraw', label: 'Withdrawals', perm: 'withdraw.read', icon: IconWithdraw, badge: 'pendingWithdraws' },
    { href: '/admins', label: 'Admin Users', role: 'super_admin', icon: IconShield },
    { href: '/settings', label: 'Settings', perm: 'settings.read', icon: IconSettings },
...
export default function AdminShell({ admin, onLogout, children, title, pendingCounts = {} }) {
    const router = useRouter();
    const [sidebarOpen, setSidebarOpen] = useState(false);

    const navItems = NAV.filter((i) => canShow(i, admin) && hasPerm(admin, i.perm));
            ...
            >
                ...
                <div className="px-5 py-5 border-b border-border">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="font-bold text-sm text-slate-100 tracking-wide">Weyra Bingo</div>
                            <div className="text-[10px] text-muted font-medium uppercase tracking-widest">Admin Panel</div>
                        </div>
                    </div>
                </div>
                ...
                <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
                    <div className="text-[10px] text-muted font-semibold uppercase tracking-widest px-3 mb-2">Navigation</div>
                    {navItems.map((item) => {
                ...
                <div className="px-3 py-4 border-t border-border space-y-3">
                    <div className="flex items-center gap-3 px-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className={`text-[10px] font-semibold uppercase tracking-wider ${roleColor}`}>
                                <IconDot size={6} className="inline mr-1" />{roleLabel}
                            </div>
                        </div>
                    </div>
                    <button
