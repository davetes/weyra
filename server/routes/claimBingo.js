...
async function handleClaimBingo(req, res, io) {
    try {
        ...
        let picks = [];
        try { picks = JSON.parse(req.body.picks || '[]'); } catch (_) { picks = []; }
        ...
        if (!result) {
            ...
            io.to(`game_${stake}`).emit('message', { type: 'disqualified', tid: tidNum });
            return res.json({ ok: false, disqualified: true, error: 'No valid bingo' });
        }
        ...
        // Broadcast winner
        const winnerName = player.username || player.phone || `Player ${player.telegramId}`;
