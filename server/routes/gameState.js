...
async function handleGameState(req, res, io) {
    try {
        ...
        if (game.countdownStartedAt && !started) {
            ...
            if (countdownRemaining <= 0) {
                ...
                if (adminTid) {
                    ...
                    if (adminSel) {
                        const biased = ensureAdminWins(game, adminSel.index, freshSelections);
                ...
                if (!game.stakesCharged) {
                    ...
                    for (const sel of freshSelections) {
                        const player = await prisma.player.findUnique({ where: { id: sel.playerId } });
                        if (player) {
                            ...
                            await prisma.player.update({
                                where: { id: player.id },
                                data: {
                                    wallet: { decrement: parseFloat(deductFromWallet.toString()) },
        ...
        if (started && game.sequence) {
            try { sequence = JSON.parse(game.sequence); } catch (_) { sequence = []; }
            ...
            currentCall = calledNumbers.length > 0 ? calledNumbers[calledNumbers.length - 1] : null;
        ...
        const totalGames = await prisma.game.count({ where: { startedAt: { not: null } } });

        const taken = freshSelections.map((s) => s.index);

        const playersDisplay = game.stakesCharged ? game.chargedCount : acceptedCount;
        ...
        return res.json({
            ...
            taken,
            countdown_started_at: game.countdownStartedAt ? game.countdownStartedAt.toISOString() : null,
