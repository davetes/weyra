...
function setupAdmin(bot) {
  ...
  bot.onText(/\/username (.+)/, async (msg, match) => {
    ...
    const adminId = getAdminChatId();
    if (!adminId)
      return bot.sendMessage(msg.chat.id, "Admin player record not found");
  ...
  bot.onText(/\/present$/, async (msg) => {
    ...
    await bot.sendMessage(
      msg.chat.id,
      `Players present (last 2 min): ${present}\nTotal registered: ${players.length}`,
    );
  });
  ...
  bot.onText(/\/top10$/, async (msg) => {
    ...
    if (!players.length)
      return bot.sendMessage(msg.chat.id, "No players found");
  ...
  bot.onText(/\/(topdaily|topweekly)$/, async (msg, match) => {
    ...
    if (period === "daily") {
      ...
      start = new Date(
        startOfToday.getTime() - mondayOffset * 24 * 60 * 60 * 1000,
      );
    ...
    const rows = [...agg.entries()]
      ...
      .sort((a, b) => b.count - a.count || b.amount.minus(a.amount).toNumber())
      .slice(0, 10);

    const lines = rows.map((r, i) => {
      const name =
        (r.player && r.player.username) ||
        (r.player && r.player.telegramId) ||
        "-";
      return `${i + 1}. ${name} — ${r.count} wins — ${r.amount.toFixed(2)} ETB`;
    });
    const title =
      period === "daily" ? "Top winners TODAY" : "Top winners THIS WEEK";
  ...
  bot.onText(/\/post(.*)/, async (msg, match) => {
    ...
    const hasReplyPhoto = !!(
      ...
      msg.reply_to_message.photo.length
    );
    if (!args.length && !hasReplyPhoto) {
      await bot.sendMessage(
        msg.chat.id,
        "Usage: /post <message> | /post photo <url> [caption] | /post file <path> [caption] | /post playnow [caption] | reply to a photo with /post [caption]",
      );
      return;
    }
    ...
    if (args[0] && args[0].toLowerCase() === "photo" && args[1]) {
      ...
      const sent = await bot.sendPhoto(msg.chat.id, filePath, {
        caption,
        parse_mode: "Markdown",
      });
      const fileId =
        sent && sent.photo && sent.photo.length
          ? sent.photo[sent.photo.length - 1].file_id
          : null;
      if (!fileId)
        return bot.sendMessage(msg.chat.id, "Could not upload photo.");
      mode = "photo";
      photoUrl = fileId;
      ...
      message &&
      /https?:\/\/.+\.(jpg|jpeg|png|gif|webp)(\?.*)?$/i.test(message)
    ) {
      mode = "photo";
      photoUrl = message;
      caption = null;
    ...
    const tids = [
      ...new Set(players.map((p) => Number(p.telegramId)).filter(Boolean)),
    ];
