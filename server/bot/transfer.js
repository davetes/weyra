...
function setupTransfer(bot, userState) {
    bot.onText(/\/transfer/, async (msg) => {
        ...
        await bot.sendMessage(chatId, 'Enter the phone number of the person you want to transfer money to 📞:', {
    ...
    bot.on('message', async (msg) => {
        ...
        if (state.transferStep === 'phone') {
            ...
            if (!recipient) {
                await bot.sendMessage(chatId, 'No registered user found with that phone number.');
                return;
            }
            if (recipient.telegramId === BigInt(tid)) {
                await bot.sendMessage(chatId, 'You cannot transfer to yourself. Please enter a different phone number:');
                return;
            }

            state.transfer = { recipientTid: recipient.telegramId, recipientPhone: recipient.phone };
            state.transferStep = 'amount';
            await bot.sendMessage(chatId, 'Here are the min and max amount you can transfer\n' +
        ...
        if (state.transferStep === 'amount') {
            ...
            if (!recipientTid) {
                await bot.sendMessage(chatId, 'Session expired. Please run /transfer again.');
                userState.delete(tid);
                return;
            }
            ...
            const sender = await prisma.player.findUnique({ where: { telegramId: BigInt(tid) } });
            const recipient = await prisma.player.findUnique({ where: { telegramId: recipientTid } });
            if (!sender || !recipient) {
            ...
            if (sbal.lt(amount)) {
                await bot.sendMessage(chatId, "You don't have a sufficient amount to withdraw. Please try again.");
                userState.delete(tid);
                return;
            }

            const newSenderBal = sbal.minus(amount);
            const newRecipientBal = new Decimal(recipient.wallet.toString()).plus(amount);
