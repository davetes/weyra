...
async function resolvePlayer(identifier) {
    ...
    if (ident.startsWith('@')) {
        ...
        return prisma.player.findFirst({ where: { username: { equals: uname, mode: 'insensitive' } } });
    }
...
function setupEntertainer(bot) {
    ...
    bot.onText(/\/add (.+)/, async (msg, match) => {
        ...
        if (parts.length < 2) return bot.sendMessage(msg.chat.id, 'Usage: /add <id|@username> <amount>');

        const player = await resolvePlayer(parts[0]);
        if (!player) return bot.sendMessage(msg.chat.id, 'Player not found');

        let amount;
        try { amount = new Decimal(parts[1]); } catch (_) { amount = null; }
        if (!amount) return bot.sendMessage(msg.chat.id, 'Invalid amount');
        if (amount.isZero()) return bot.sendMessage(msg.chat.id, 'Amount must be non-zero');

        const before = new Decimal(player.wallet.toString());
        const after = before.plus(amount);
        await prisma.$transaction([
            prisma.player.update({ where: { id: player.id }, data: { wallet: after.toNumber() } }),
        ...
        await bot.sendMessage(msg.chat.id, `Wallet updated: ${before.toFixed(2)} → ${after.toFixed(2)} ETB`);
    });
    ...
    bot.onText(/\/subtract (.+)/, async (msg, match) => {
        ...
        const parts = match[1].trim().split(/\s+/);
        if (parts.length < 2) return bot.sendMessage(msg.chat.id, 'Usage: /subtract <id|@username> <amount>');
        ...
        try { amount = new Decimal(parts[1]); } catch (_) { amount = null; }
        if (!amount) return bot.sendMessage(msg.chat.id, 'Invalid amount');
        if (amount.lte(0)) return bot.sendMessage(msg.chat.id, 'Amount must be greater than zero');
